    ...
    <div className="border-b sticky top-0 z-50 bg-white/[95%]  ">
      <div className="flex justify-between items-center sm:mx-6 md:mx-10 lg:mx-12 ">
        ...
        <div className="flex items-center pr-3  font-semibold text-gray-600">
          {localStorage.getItem('access_token') && (<button onClick={() => { props.onRentClick(true) }} className="text-[17px]">Rent House</button>)}
          <div className="flex items-center mx-8 gap-1">
            <BiWorld className="" />
            <div className="">EN</div>
          </div>

          {!localStorage.getItem('access_token') && (<button onClick={() => { props.onSignUpClick(true) }} className="flex items-center border px-3 py-2 rounded-full gap-2 bg-[#ff5a60] text-white font-bold shadow-lg shadow-gray-300 hover:bg-[#f9787c] duration-100 ease-out">
            <p>Sign in</p>
